...
<? if( ! isset( $hide_btns ) ): ?> 
 	<? if( $type == 'ins' ): ?>
		<a href="<?= safe_url( '/agent/reports', 'submit_master_agent_report', array( $ins_id ) ); ?>" class="btn-create">
	<? else: ?>
		<a href="<?= safe_url( '/agent/reports', 'submit_master_agent_report', array( $ins_id,  $totals[ 'company' ]  ) ); ?>" class="btn-create">
	<? endif; ?>		

		<? if( $sub_agent ): ?>
			<span>Submit Report To Master Agent</span>
		<? else: ?>
			<span>Submit Report To <?= $agency_name; ?></span>
		<? endif; ?>
		</a>
	<a target="_blank" href="/_tasks/print_master_report.cfm?var1=<?= $ins_id; ?>&var2=<?= $this->session->userdata( 'mek' ); ?>&var4=<?= $sub_agent; ?>&var5=<?= $type; ?>" class="btn-list"><span>Print Report</span></a>
<? else: ?>

<a href="agent/reports/list_sub_agent_reports/<?= $sub_agent; ?>/<?= $ins_id; ?>" class="btn"><span>Back to Report List</span></a>
