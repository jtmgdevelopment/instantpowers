...
<fieldset>
	...
	<div class="col50">
		<label>Amount</label>
		<p class="row">$<?= number_format( $hidden_amount, 2, '.', ',' ); ?></p>
	</div>
</fieldset>		
...
<?= form_open('/agent/pay_premium/process_report_premium'); ?>

<input type="hidden" name="ins_id" value="<?= $ins_id ?>" />
<input type="hidden" name="is_mga" value="<?= $is_mga ?>" />
...
<input type="hidden" name="amount" value="<?= $hidden_amount ?>" />
<input type="hidden" name="orig_amount" value="<?= $hidden_amount ?>" />
