<?= form_open('/admin/manage_jail/process_jail_admin_frm'); ?>
<input type="hidden" name="existing_username" value="<?= set_field( $jail_admin, 'email' ); ?>" />
<input type="hidden" name="existing_email" value="<?= set_field( $jail_admin, 'email' ); ?>" />
<input type="hidden" name="update" value="<? $update; ?>" />
<section>
<fieldset>
	<legend>Jail Administrator Information</legend>
		<div class="col50">
		<?
			...
			$opts = array(
				'name' 	=> 'first_name',
				'id'	=> 'first_name',
				'value'	=> set_field( $jail_admin, 'first_name' )
								
			);
			echo form_label('First Name*', 'first_name'); 
			echo form_input($opts);
			echo form_hidden('mek', set_field( $jail_admin , 'mek' ));
		?>
		</div>
		<div class="col50">
		<?
			...
			$opts = array(
				'name' 	=> 'last_name',
				'id'	=> 'last_name',
				'value'	=> set_field( $jail_admin, 'last_name' )
...
<fieldset>
	<legend>Jail Information</legend>	
		<div class="col50">
		<?
			...
			$opts = array(
				'name' 	=> 'email',
				'id'	=> 'email',
				'value'	=> set_field( $jail_admin, 'email' )
								
			);
			echo form_label('Jail email*', 'email'); 
			echo form_input($opts);
		...
		<?
			...
			$opts = array(
				'name' 	=> 'jail_name',
				'id'	=> 'jail_name',
				'value'	=> set_field( $jail_admin, 'jail_name' )
		...
		<?
			...
			$opts = array(
				'name' 	=> 'address',
				'id'	=> 'address',
				'value'	=> set_field( $jail_admin, 'address' )
		...
		<?
			...
			$opts = array(
				'name' 	=> 'city',
				'id'	=> 'city',
				'value'	=> set_field( $jail_admin, 'city' )
		...
		<?
			...
			echo state_dropdown('state', set_value('state'), set_field( $jail_admin, 'state' ));		
		...
		<?
			...
			$opts = array(
				'name' 	=> 'zip',
				'id'	=> 'zip',
				'value'	=> set_field( $jail_admin, 'zip' )
...
<div class="t-right">
	<p>
		<button type="button" onClick="location.href = '/admin/manage_jail/';"class="white">Cancel</button>
