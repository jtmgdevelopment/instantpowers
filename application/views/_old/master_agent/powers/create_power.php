...
<?= form_open('/master_agent/powers/process_step_one'); ?>
...
<fieldset id="extended">
	...
	<? else: ?>
		<div class="col50">
		<?
			echo form_label('Defendant Name','defendant_name');		
		?>
		<p class="row"><strong><?= $has_defendant;?></strong></p>
		</div>
	<? endif; ?>	
	<div class="clearfix"></div>	
	<? if( empty( $has_agent ) ): ?>
	...
	<? else: ?>
		...
		<div class="col50">
			<?
				echo form_label('Executing Agent','agent');		
			?>
			<p class="row"><strong><?= $has_agent;?></strong></p>
		</div>	
...
<div class="t-right">
	<p>
		...
		<? endif; ?>
		<button type="button" class="white" onClick="location.href = '/master_agent/powers/cancel_power/<?= $transmission->trans_id; ?>/<?= $powers->pek ?>'">Cancel</button>
	</p>
</div>
</section>
<?= form_close(); ?>
