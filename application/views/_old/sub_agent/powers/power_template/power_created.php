...
<div id="page_wrap">
	...
	<div id="nav">
		<a href="/sub_agent/power_inventory">Back To Inventory</a> | <a href="/generate_power_pdf.cfm?power=<?= $batch; ?>&trans_id=<?= $trans_id; ?>&batch=<?= $is_batch; ?>&prefix_id=<?= $powers->prefix_id; ?>">Print Power/s</a>
	</div>
	<br>
	<br>

	<div align="center">
		<button onClick="location.href = '/sub_agent/powers/step_three/<?= $trans_id; ?>/<?= $powers->pek; ?>';">Back To Transmission</button>&nbsp;&ndash;&nbsp;
		<button onClick="location.href = '/sub_agent/powers/execute_power/<?= $trans_id; ?>/<?= $powers->pek; ?>';">Execute Power</button>&nbsp;&ndash;&nbsp;
		<button onClick="location.href = '/sub_agent/powers/print_power/<?= $trans_id; ?>/<?= $is_batch; ?>/<?= $powers->prefix_id; ?>';">Print Power</button>
	...
	<div id="fullwidth">
		...
		<div id="power">
			...
			<?= convert_number_to_words( preg_replace("/[^0-9]/","", number_format( str_replace('$','', $powers->amount ) ) ) ); ?>
			(
			<?= $powers->amount; ?>
		...
		<div class="one-half-two first">
			<p>
				...
				&nbsp;&nbsp; <strong>
				<?= $p->defendant->first_name . ' ' . $p->defendant->last_name; ?>
				</strong><br />
	...
	<div id="fullwidth">
		...
		<div class="one-half first">
			...
			<p>
				POWER #&nbsp;&nbsp; <strong><?= $p->power->pek; ?></strong>
				<br />
				ARREST #&nbsp; <strong><?= $p->power->case_number; ?></strong>
			</p>
		</div>
		<div class="one-half">
		...
		<div class="" style="clear:both;">
			<div style=" text-align:center;">
				...
				<h4 style="font-size:20px; margin-top:10px">
					<strong><?= $p->defendant->first_name; ?> <?= $p->defendant->last_name; ?></strong>
				</h4>
		...
		<div class="">
			<div style=" font-size:13px;">
				IN THE <strong><?= $p->power->court; ?> Court</strong><br>
				<strong><?= $p->power->county; ?></strong> County, STATE OF FLORIDA
			</p>
			</div>
		</div>
		<div class="inside-wrap">
			<p>
				KNOW ALL MEN BY THESE PRESENT: That we the above named defendant as principal, and PALMETTO SURETY CORPORATION, a SOUTH CAROLINA CORPORATION, surety are held and firmly bound unto the Governor of the State of Florida, and his successors in office, the said principal, in the sum of $<strong><?= $p->power->bond_amount; ?></strong> and the said surety for the life amount, for the payment whereof well and truly to be made we bind ourselves, our heirs, executors, administrators and assigns firmly by these presents.
			</p>
			<p>
				Signed and sealed this <strong><?= date('jS'); ?> day of <?= date('F'); ?> A.D.. <?= date('Y'); ?></strong>
				<br />
				The condition of this obligation is such that if said principal shall appear on <strong><? if( strlen( $p->power->court_date ) ): ?><?= $p->power->court_date; ?><? else: ?>___________________<? endif; ?></strong>
				at the next Regular or Special term of the above court and shall submit to the said Court to answer a charge of: <strong><?= $p->power->charge; ?></strong>; and shall submit to orders and process of said court and not depart the same without leave, then this obligation to be void, else to remain in full force and virtue.
			</p>
		</div>
		<div class="one-half first">
