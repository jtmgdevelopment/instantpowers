	...
	*/		
	...
	{	
		...
		switch($flag){
			...
			case 'JS':
			case 'js':
				return $this->_display_js();
			break;
			
			case 'CSS':
			case 'css':
				return $this->_display_css();
			break;
			
			case 'both':
				return $this->_display_css().$this->_display_js();
			break;
	...
	private function _display_js($group = 'main')
	{
		...
		// if we're combining files and minifying them
			...
			foreach($this->js[$group] as $ref):
				...
				if(!$ref['combine']):
					$s[] = (isset($ref['prod'])) ? $this->_tag('js', $ref['prod']) : $this->_tag('js', $ref['dev']);					
				elseif(!$ref['minify']):
		...
		if($this->dev){ // we're in a development environment
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					$css[] = $this->_tag('css', $ref['dev'], FALSE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach ($refs as $ref):
					...
					if(!$ref['combine']):
						$css[] = (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], $media) : $this->_tag('css', $ref['dev'], $media);
					elseif(!$ref['minify']):
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'], 'minify'=>$ref['minify'] ) : array('dev'=>$ref['dev'], 'minify'=>$ref['minify']);
					else:
						$files[] = (isset($ref['prod'])) ? array('prod'=>$ref['prod'], 'dev'=>$ref['dev'] ) : array('dev'=>$ref['dev']);
				...
				$css[] = $this->_tag('css',  $filename, TRUE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					$css[] = $this->_tag('css', $f, TRUE, $media);
			...
			foreach($this->css[$group] as $media => $refs):
				...
				foreach($refs as $ref):
					...
					$f = (isset($ref['prod'])) ? $ref['prod'] : $ref['dev'];
					$css[] = $this->_tag('css', $f, FALSE, $media);
