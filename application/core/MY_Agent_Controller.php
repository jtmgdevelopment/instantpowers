...
class Master_Agent_Controller extends MY_Controller {

   public function __construct()
	{
		parent::__construct();
 
		if ( ! $this->session->userdata('logged_in') && $this->session->userdata('role') != 'master_agent')
        {
		...
		if( ! $this->session->userdata( 'run_expiration' ) )
		{
			...
			curl_setopt($ch, CURLOPT_URL, "http://is.bailcommerce.com/_tasks/expire_transmissions.cfm?username=" . $this->session->userdata( 'username' ) );
